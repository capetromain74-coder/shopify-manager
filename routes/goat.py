...
def api_goat_debug_algolia():
    ...
    return jsonify({'sku': sku, 'hit_sku': best.get('sku'), 'slug': best.get('slug'), 'all_keys': all_keys, 'image_fields': image_fields, 'total_hits': len(hits)})
...
def api_goat_scrape_images():
        ...
        try:
            ...
            def find_images(obj, depth=0):
                ...
                if isinstance(obj, str):
                    ...
                    for v in obj.values(): find_images(v, depth+1)
                elif isinstance(obj, list):
                    for item in obj: find_images(item, depth+1)
    ...
    return jsonify({'slug': slug, 'page_url': page_url, 'html_length': len(html), 'has_next_data': next_data is not None, 'images_found': len(images), 'images': images[:20], 'html_preview': html[:500] if len(html) < 5000 else f"...{len(html)} chars..."})
...
def api_goat_test_resize():
    ...
    result = {'pillow_installed': pillow_ok, 'pillow_version': pillow_version, 'url': url}
    if pillow_ok:
...
def api_goat_debug_api():
    ...
    result = {'sku': sku, 'slug': slug, 'algolia_main_picture': product.get('main_picture_url', '')}
...
def api_goat_images():
    ...
    if result.get('multi'):
        ...
        return jsonify({'multi': True, 'results': result['results']})
    if not result.get('images'):
        return jsonify({'error': 'Aucune image trouvee'}), 404
    return jsonify({'name': result.get('name', ''), 'sku': result.get('sku', sku), 'images': result.get('images', []), 'multi': False})
...
def api_goat_images_by_slug():
    ...
    if raw:
        try:
            ...
            pass
    return jsonify({'name': name, 'slug': slug, 'images': images})

@goat_bp.route('/api/goat/apply', methods=['POST'])
def api_goat_apply():
    """Remplace les images d'un produit par celles de GOAT + fix SEO images"""
    data = request.json
    ...
    try:
        ...
        clothing_kw = ['hoodie', 'sweatshirt', 'sweater', 'sweatpant', 'sweatshort', 'tee ', 't-shirt',
                       ...
                       'pullover', 'anorak', 'puffer', 'fleece', 'bomber', 'balaclava']
