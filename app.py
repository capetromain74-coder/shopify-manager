"""
Shopify Manager V4.1 - SEO Edition
- Matching précis: modèle > marque (jamais "tous les produits")
- Vérification avant modification 
- Sélection granulaire (meta title / meta desc / description)
"""
...
collections_cache = {'data': [], 'last_update': None}
    ...
    if result and 'metafields' in result:
        for mf in result['metafields']:
            if mf.get('namespace') == 'global':
                if mf.get('key') == 'title_tag': meta_title = mf.get('value')
                elif mf.get('key') == 'description_tag': meta_desc = mf.get('value')
...
def get_all_collections():
    ...
    if collections_cache['last_update']:
        ...
        if age < 600 and collections_cache['data']: return collections_cache['data']
    all_collections = []
...
def check_seo_status(product, metafields=None):
    ...
    if metafields and metafields.get('meta_title'):
        ...
        if SITE_NAME.lower() in mt.lower(): status['meta_title']['needs_update'] = False
    ...
    if body and len(body.strip()) > 100:
        ...
        status['description']['value'] = body[:200] + '...' if len(body) > 200 else body
...
def calculate_seo_score(seo_status):
    score = 0
    if not seo_status['meta_title']['needs_update']: score += 30
    elif seo_status['meta_title']['exists']: score += 15
    if not seo_status['meta_description']['needs_update']: score += 30
    elif seo_status['meta_description']['exists']: score += 15
    if not seo_status['description']['needs_update']: score += 40
    elif seo_status['description']['exists']: score += 20
    return score
...
def extract_sku(product):
    if product.get('variants') and product['variants']:
        return product['variants'][0].get('sku', '')
    return ''

def extract_brand(product):
    title = product.get('title', '')
    for brand in ['Adidas', 'Nike', 'Air Jordan', 'Jordan', 'New Balance', 'Puma', 'Asics', 'UGG', 'Yeezy', 'Birkenstock']:
        if brand.lower() in title.lower(): return brand
...
def generate_meta_title(product):
    ...
    if len(meta) > 60: meta = f"{title[:60 - len(SITE_NAME) - 6]}... | {SITE_NAME}"
    return meta
...
def generate_meta_description(product):
    ...
    if len(meta) > 155: meta = f"Achetez la {title} ✓ {BENEFITS[0]} ✓ {BENEFITS[1]} - {SITE_NAME}"[:155]
    return meta
...
<header class="hd"><a href="/" class="back">← Accueil</a><div class="logo">🤖 SEO <span>Manager</span></div><div></div></header>
<div class="stats"><div class="stat"><div class="sv g" id="s1">-</div><div class="sl">Complet</div></div><div class="stat"><div class="sv o" id="s2">-</div><div class="sl">Partiel</div></div><div class="stat"><div class="sv r" id="s3">-</div><div class="sl">Manquant</div></div><div class="stat"><div class="sv" id="s4">-</div><div class="sl">Total</div></div><div class="stat"><div class="sv g" id="s5">-%</div><div class="sl">Optimisé</div></div></div>
<div class="ctrl"><div class="cg"><label>Rechercher</label><input type="text" id="src" placeholder="Nom, SKU..."></div><div class="cg"><label>Filtrer</label><select id="flt"><option value="all">Tous</option><option value="missing">❌ Sans liens</option><option value="partial">⚠️ Partiel</option><option value="complete">✅ Complet</option></select></div><div class="fs"><span style="font-size:11px;color:#888">Champs:</span><label><input type="checkbox" id="ft"> Title</label><label><input type="checkbox" id="fd"> Desc</label><label><input type="checkbox" id="fb" checked> Body</label></div><button class="btn bs" onclick="load()">🔄</button><button class="btn bp" onclick="applySel()">⚡ Sélection</button><button class="btn bd" onclick="applyAll()">🚀 TOUT</button><div style="margin-left:auto;font-size:12px;color:#888"><strong id="sc">0</strong> sélect.</div></div>
...
async function load(){document.getElementById('prods').innerHTML='<div class="ld"><div class="sp"></div>Chargement...</div>';try{const r=await fetch('/api/products');const d=await r.json();P=d.products;document.getElementById('s1').textContent=d.stats.seo_complete;document.getElementById('s2').textContent=d.stats.seo_partial;document.getElementById('s3').textContent=d.stats.seo_missing;document.getElementById('s4').textContent=d.stats.total;document.getElementById('s5').textContent=d.stats.percentage_complete+'%';filter();}catch(e){document.getElementById('prods').innerHTML='<div class="ld">Erreur</div>';}}
function filter(){const s=document.getElementById('src').value.toLowerCase();const f=document.getElementById('flt').value;let L=P.filter(p=>{if(s&&!p.title.toLowerCase().includes(s)&&!(p.sku||'').toLowerCase().includes(s))return false;if(f==='missing')return !p.has_links;if(f==='complete')return p.has_links;if(f==='partial')return p.has_description&&!p.has_links;return true;});render(L);}
function render(L){if(!L.length){document.getElementById('prods').innerHTML='<div class="ld">Aucun</div>';return;}document.getElementById('prods').innerHTML=L.map(p=>{const ck=sel.has(p.id)?'chk':'';const sc=p.seo_score>=70?'h':p.seo_score>=30?'m':'l';let cc='n',ct='⚠️ Aucune';if(p.collection){cc=p.collection.type==='model'?'f':'b';ct=(p.collection.type==='model'?'✅ ':'📁 ')+p.collection.title;}return '<div class="prod"><div class="pck '+ck+'" onclick="tog('+p.id+')"></div><img class="pim" src="'+(p.image||'')+'" onerror="this.style.background=\'#333\'"><div class="pin"><h3>'+p.title.substring(0,45)+(p.title.length>45?'...':'')+'</h3><div class="psku">'+( p.sku||'N/A')+'</div><div class="pcol '+cc+'">'+ct+'</div></div><div class="pst"><div class="si '+(p.has_description?'sok':'sms')+'">'+(p.has_description?'✅':'❌')+' Desc</div><div class="si '+(p.has_links?'sok':'sms')+'">'+(p.has_links?'✅':'❌')+' Liens</div></div><div class="psc"><span class="scb '+sc+'">'+p.seo_score+'%</span></div><div class="pac"><button class="ab v" onclick="view('+p.id+')">👁️</button><button class="ab a" onclick="applyOne('+p.id+')">⚡</button></div></div>';}).join('');}
function tog(id){sel.has(id)?sel.delete(id):sel.add(id);document.getElementById('sc').textContent=sel.size;filter();}
async function view(id){curId=id;document.getElementById('mmb').innerHTML='<div class="ld"><div class="sp"></div></div>';document.getElementById('mod').classList.add('sh');try{const r=await fetch('/api/product/'+id+'/seo-status');const d=await r.json();const s=d.seo_status,g=d.generated;let h='<p><b>Produit:</b> '+d.product.title+'</p>';if(d.collection)h+='<p><b>Collection:</b> '+d.collection.title+' ('+d.collection.type+')</p>';h+='<p><b>Score:</b> '+d.score+'%</p><hr style="border-color:#333;margin:15px 0">';h+='<div class="ss"><h4>Meta Title <span class="bg '+(s.meta_title.needs_update?'ms':'ok')+'">'+(s.meta_title.needs_update?'À modifier':'OK')+'</span></h4>'+(s.meta_title.value?'<div class="sc">'+s.meta_title.value+'</div>':'<div class="sc">Aucun</div>')+'<div class="sg">'+g.meta_title+'</div></div>';h+='<div class="ss"><h4>Meta Desc <span class="bg '+(s.meta_description.needs_update?'ms':'ok')+'">'+(s.meta_description.needs_update?'À modifier':'OK')+'</span></h4>'+(s.meta_description.value?'<div class="sc">'+s.meta_description.value+'</div>':'<div class="sc">Aucune</div>')+'<div class="sg">'+g.meta_description+'</div></div>';h+='<div class="ss"><h4>Description <span class="bg '+(s.description.needs_update?'ms':'ok')+'">'+(s.description.needs_update?'À modifier':'OK')+'</span>'+(s.description.has_links?' 🔗':'')+'</h4>'+(s.description.value?'<div class="sc">'+esc(s.description.value)+'</div>':'<div class="sc">Aucune</div>')+'<div class="sg"><pre>'+esc(g.description)+'</pre></div></div>';h+='<div class="mfs"><h4>Appliquer:</h4><label><input type="checkbox" id="mft" '+(s.meta_title.needs_update?'checked':'')+'> Meta Title</label><label><input type="checkbox" id="mfd" '+(s.meta_description.needs_update?'checked':'')+'> Meta Desc</label><label><input type="checkbox" id="mfb" '+(s.description.needs_update?'checked':'')+'> Description</label></div>';document.getElementById('mmb').innerHTML=h;document.getElementById('mt').textContent=d.product.title;}catch(e){document.getElementById('mmb').innerHTML='<p>Erreur</p>';}}
function esc(t){return(t||'').replace(/</g,'&lt;').replace(/>/g,'&gt;');}
function closeMod(){document.getElementById('mod').classList.remove('sh');curId=null;}
async function applyMod(){if(!curId)return;const f=[];if(document.getElementById('mft').checked)f.push('meta_title');if(document.getElementById('mfd').checked)f.push('meta_description');if(document.getElementById('mfb').checked)f.push('description');if(!f.length){toast('Sélectionnez un champ','e');return;}closeMod();toast('Application...','s');try{const r=await fetch('/api/seo/apply',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({product_id:curId,fields:f})});if((await r.json()).success){toast('OK!','s');load();}}catch(e){toast('Erreur','e');}}
...
async function applyOne(id){const f=getF();if(!f.length){toast('Cochez un champ','e');return;}toast('...','s');try{const r=await fetch('/api/seo/apply',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({product_id:id,fields:f})});if((await r.json()).success){toast('OK!','s');load();}}catch(e){toast('Erreur','e');}}
...
async function batch(ids,f){showPB();await fetch('/api/seo/batch',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({product_ids:ids,fields:f})});mon();}
function mon(){const iv=setInterval(async()=>{const r=await fetch('/api/progress');const p=await r.json();document.getElementById('pf').style.width=(p.current/p.total*100)+'%';document.getElementById('pc').textContent=p.current+'/'+p.total;document.getElementById('px').textContent=p.message;if(!p.running){clearInterval(iv);hidePB();toast(p.message,'s');sel.clear();load();}},800);}
...
function toast(m,t){const o=document.querySelector('.tst');if(o)o.remove();const e=document.createElement('div');e.className='tst '+t;e.textContent=m;document.body.appendChild(e);setTimeout(()=>e.remove(),4000);}
...
def api_collections():
    return jsonify({'collections': get_all_collections(), 'count': len(get_all_collections())})
...
def api_products():
    ...
    for p in products:
        ...
        has_desc = body and len(strip_html(body)) > 100
        has_links = '<a href=' in (body or '').lower() and SITE_DOMAIN in (body or '').lower()
        ...
        result.append({'id': p['id'], 'title': p['title'], 'handle': p['handle'], 'image': p.get('image', {}).get('src') if p.get('image') else None, 'sku': extract_sku(p), 'collection': collection, 'seo_score': score, 'has_description': has_desc, 'has_links': has_links})
...
def api_product_seo_status(product_id):
    ...
    return jsonify({'product': {'id': product['id'], 'title': product['title'], 'sku': extract_sku(product)}, 'collection': collection, 'seo_status': seo_status, 'score': calculate_seo_score(seo_status), 'generated': {'meta_title': generate_meta_title(product), 'meta_description': generate_meta_description(product), 'description': generate_description(product, collection)}})

@app.route('/api/seo/apply', methods=['POST'])
def api_apply_seo():
    data = request.json
    product_id, fields = data.get('product_id'), data.get('fields', [])
    if not product_id or not fields: return jsonify({'error': 'Missing params'}), 400
    result = shopify_request(f'products/{product_id}.json')
    if not result: return jsonify({'error': 'Not found'}), 404
    product = result['product']
    collection = find_best_collection(product.get('title', ''), get_all_collections())
    updates = {}
    if 'meta_title' in fields: updates['meta_title'] = generate_meta_title(product)
    if 'meta_description' in fields: updates['meta_description'] = generate_meta_description(product)
    if 'description' in fields: updates['body_html'] = generate_description(product, collection)
    return jsonify({'success': update_product_seo(product_id, updates), 'applied_fields': list(updates.keys())})

@app.route('/api/seo/batch', methods=['POST'])
def api_batch_seo():
    global task_progress
    data = request.json
    product_ids, fields = data.get('product_ids', []), data.get('fields', ['description'])
    if not product_ids: return jsonify({'error': 'No products'}), 400
    def process():
        global task_progress
        task_progress = {'running': True, 'current': 0, 'total': len(product_ids), 'message': 'Start...', 'success_count': 0, 'error_count': 0}
        collections = get_all_collections()
